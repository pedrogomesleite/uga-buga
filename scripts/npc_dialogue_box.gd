...
@onready var conteiner = $MarginContainer
...
func iniciar_dialogo(conteudo: String, time = 10.0):
	if not started:
		...
		tween_out.tween_property(conteiner, "modulate:a", 0.0, time)
		tween_out.tween_property(rect, "modulate:a", 0.0, time)
		await tween_out.finished
		fim_dialogo.emit()
		conteiner.queue_free()
		rect.queue_free()
